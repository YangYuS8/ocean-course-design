# 分工 3：PHP 单表结构、基础模型与种子数据讲解（邢文博）

## 负责定位

这一部分是 PHP 后端中相对容易、风险较低的部分。你不需要讲复杂业务逻辑，也不需要解释所有 Laravel 架构，只需要把“数据库表、字段、简单模型关系、初始数据”讲清楚。

如果老师追问到整体架构、异常判断、登录中间件等难点，不要指望杨栋森兜底。你可以先明确自己负责数据库和基础模型，再把问题交给对应模块负责人回答。
...
这些如果被深问，由林宇涵补充；你只需要说明它们也通过 `sample_id` 关联样本。
...
> 我主要负责数据库表和基础模型关系，这个问题涉及后端接口/业务逻辑，应该由林宇涵回答。
