# 分工 2：PHP 核心业务控制器与异常分析逻辑（林宇涵）

## 负责定位

这一部分是全组另一个较难的 PHP 部分，建议分给除杨宇外基础最好的组员。它负责解释系统核心业务逻辑：任务、样本、检测结果、异常处理、分析建议。

注意：这部分由林宇涵自己答辩，不能依赖杨栋森兜底。杨栋森只负责后端总体架构、路由入口、登录保护和用户管理；本文件中的任务、样本、检测结果、异常处理和分析逻辑需要林宇涵讲清楚。
...
- 开始任务。
- 提交任务。
- 删除任务，并通过数据库级联清理任务下的样本、检测结果、异常和分析记录。
...
8. 路由模型绑定：URL 中的 `{task}`、`{sample}`、`{result}` 会自动转换为对应模型。
9. 级联删除：删除任务或样本时，数据库会自动清理下级数据，避免孤立记录。
