...
本次增强后的样本详情、首页近期分析建议、分析报告摘要展示，属于较难的前后端集成点，统一归你负责。基础较弱组员只需要知道这些功能有什么用，不需要深入讲 React 状态和接口刷新细节。
...
8. 生成分析建议，说明规则分析逻辑和 `report_summary` 报告摘要。
9. 打开样本详情，说明一个样本下面可以看到检测结果、异常记录和分析报告。
10. 返回首页，说明统计数据和近期分析建议实时变化。
...
- 为什么异常判断放在后端？
- 前端页面的数据从哪里来？
- 样本详情为什么能一次展示任务、检测结果、异常和分析？
- `report_summary` 和 `summary` / `suggestion` 有什么区别？
- 如何证明不是静态页面？
