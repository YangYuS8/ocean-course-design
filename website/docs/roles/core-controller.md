---
title: 林宇涵：核心控制器负责人
description: 给林宇涵准备的核心 PHP 控制器学习路线。
---
...
处理巡检任务：查询、创建、修改、开始、提交和删除。

要会说：

> 创建任务时，后端校验任务名称、区域、负责人和计划日期，然后写入 `inspection_tasks` 表。修改任务用于修正录入错误；删除任务时，任务下的样本、检测结果、异常和分析记录会被级联清理。
...
> 样本必须关联一个真实存在的任务，所以后端用 `exists:inspection_tasks,id` 校验。修改样本用于修正采样信息；删除样本时，该样本下的检测结果、异常和分析记录会一起清理。

### ResultController

处理检测结果，这是重点。它支持录入、修改、删除，并在录入或修改后重新判断是否异常。

要会说：

> 检测结果提交或修改后，后端根据参考下限和上限判断是否异常。如果异常，会自动生成一条待处理异常记录。删除检测结果后，后端会重新计算样本状态。
...
> 分析不是 AI，而是规则分析：统计异常指标和待处理异常数量，根据情况生成建议，并生成报告摘要供样本详情页展示。
...
> 根据检测结果数量、异常指标数量和待处理异常数量，用 if 判断生成不同建议；报告摘要会整理样本编号、采样点、采样人、天气、坐标和异常数量。
